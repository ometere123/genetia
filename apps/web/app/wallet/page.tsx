  ...
  return <main className="flex-1 bg-surface-0 px-4 py-8 text-slate-100 sm:px-6 sm:py-10">
    <section className="mx-auto max-w-[640px]">
      <header className="mb-6 flex items-center justify-between gap-4">
        <div><p className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-brand-light"><Icon name="wallet" className="h-3.5 w-3.5" /> {t("wallet.userOwned")}</p><h1 className="mt-1 text-2xl font-bold tracking-tight text-white">{t("wallet.title")}</h1><p className="mt-1 text-xs text-slate-500">{t("wallet.description")}</p></div>
        <button type="button" onClick={refresh} disabled={!wallet || refreshing} aria-label={t("wallet.refresh")} className="grid h-9 w-9 shrink-0 place-items-center rounded-lg border border-border bg-surface-1 text-slate-400 transition hover:text-white disabled:opacity-50"><span className={refreshing ? "animate-spin" : ""}>↻</span></button>
      </header>

      {!authenticated ? <section className="rounded-2xl border border-border bg-surface-1 p-8 text-center"><p className="text-sm text-slate-400">{t("wallet.login")}</p><button type="button" onClick={() => login()} className="mt-5 rounded-lg bg-brand px-4 py-2.5 text-sm font-semibold text-white">{t("wallet.connect")}</button></section> : <>
        <section className="overflow-hidden rounded-2xl border border-border bg-[radial-gradient(ellipse_at_top_right,rgba(99,102,241,0.11),transparent_55%),linear-gradient(145deg,rgb(var(--surface-1)),rgb(var(--surface-0)))] p-5 sm:p-6">
          <div className="flex items-start justify-between gap-4"><div><p className="text-[11px] text-slate-500">{t("wallet.balance")}</p><p className="mt-1 text-3xl font-semibold tracking-tight text-white">{balance === undefined ? "—" : usdcDisplay(balance)}</p></div><span className={`rounded-full border px-2.5 py-1 text-[10px] font-medium ${onBaseSepolia ? "border-emerald-900/70 bg-emerald-950/30 text-emerald-300" : "border-amber-900/70 bg-amber-950/30 text-amber-300"}`}>{onBaseSepolia ? "Base Sepolia" : t("wallet.wrongNetwork")}</span></div>
          <div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-xl border border-border bg-surface-0/70 p-3"><p className="text-[10px] text-slate-500">{t("wallet.walletType")}</p><p className="mt-1 text-xs font-medium capitalize text-slate-200">{wallet?.walletClientType ?? "Privy wallet"}</p></div><div className="rounded-xl border border-border bg-surface-0/70 p-3"><p className="text-[10px] text-slate-500">{t("wallet.recentTrades")}</p><p className="mt-1 text-xs font-medium text-slate-200">{records.length}</p></div></div>
          {wallet && <div className="mt-4 flex items-center justify-between gap-3 rounded-xl border border-border bg-surface-0/70 px-3 py-2.5"><span className="truncate font-mono text-[11px] text-slate-400">{wallet.address}</span><button type="button" onClick={() => { void navigator.clipboard?.writeText(wallet.address); setMessage(t("wallet.copied")); }} className="shrink-0 text-[11px] font-medium text-brand-light hover:text-white">{t("wallet.copy")}</button></div>}
          {!onBaseSepolia && <button type="button" onClick={() => void changeNetwork()} className="mt-4 w-full rounded-lg border border-amber-900/70 bg-amber-950/20 px-3 py-2.5 text-xs font-semibold text-amber-200">{t("wallet.switchNetwork")}</button>}
        </section>

        <nav aria-label="Wallet sections" className="mt-5 grid grid-cols-3 overflow-hidden rounded-xl border border-border bg-surface-1 text-center text-xs"><Link href="/wallet" className="border-b-2 border-brand px-2 py-3 font-medium text-brand-light">{t("wallet.overview")}</Link><Link href="/portfolio" className="px-2 py-3 text-slate-400 transition hover:bg-surface-2 hover:text-white">{t("wallet.positions")}</Link><Link href="/portfolio#history" className="px-2 py-3 text-slate-400 transition hover:bg-surface-2 hover:text-white">{t("wallet.history")}</Link></nav>

        <div className="mt-4 grid grid-cols-2 gap-3"><a href="https://faucet.circle.com/" target="_blank" rel="noreferrer" className="rounded-xl border border-brand/30 bg-brand-muted/30 p-4 text-center transition hover:border-brand/60"><span className="mx-auto grid h-7 w-7 place-items-center rounded-lg bg-brand/15 text-brand-light"><Icon name="download" className="h-4 w-4" /></span><span className="mt-2 block text-sm font-semibold text-slate-100">{t("wallet.getUsdc")}</span><span className="mt-1 block text-[11px] text-slate-500">{t("wallet.faucet")}</span></a><Link href="/portfolio" className="rounded-xl border border-border bg-surface-1 p-4 text-center transition hover:bg-surface-2"><span className="mx-auto grid h-7 w-7 place-items-center rounded-lg bg-surface-3 text-slate-300"><Icon name="trend" className="h-4 w-4" /></span><span className="mt-2 block text-sm font-semibold text-slate-100">{t("wallet.viewPortfolio")}</span><span className="mt-1 block text-[11px] text-slate-500">{t("wallet.positionsActivity")}</span></Link></div>
        {loading && <p className="mt-4 text-center text-xs text-slate-500">{t("wallet.loading")}</p>}
        ...
        <p className="mt-4 text-center text-[11px] leading-5 text-slate-600">{t("wallet.heldByUser")}</p>
      </>}
    </section>
  </main>;
}
