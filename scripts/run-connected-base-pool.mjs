...
const fixture = readJson("contracts/genlayer/fixtures/connected_pool_live.json");
const base = readJson("deployments/base-sepolia.json");
const genlayer = readJson("deployments/genlayer-connected-live.json");
const outPath = path.join(root, "deployments/base-connected-pool.json");
...
function save(patch) {
  const prior = fs.existsSync(outPath) ? readJson("deployments/base-connected-pool.json") : {};
...
save({ status: "FUNDED_BOTH_SIDES", market, createTx: (fs.existsSync(outPath) ? readJson("deployments/base-connected-pool.json").createTx : undefined), approvalTx: approvalTx ?? readJson("deployments/base-connected-pool.json").approvalTx, yesStakeTx: yesTx ?? readJson("deployments/base-connected-pool.json").yesStakeTx, noStakeTx: noTx ?? readJson("deployments/base-connected-pool.json").noStakeTx, yesTotal: yesTotal.toString(), noTotal: noTotal.toString(), usdcAfter: formatUnits(balanceAfter, 6), closeTime: fixture.close_time, resolutionAvailableTime: fixture.resolution_available_time, terminalDeadline: fixture.absolute_terminal_deadline, lastBaseBlock: block.number.toString() });
console.log(JSON.stringify({ chainId, market, marketId: fixture.market_id, resolver: genlayer.resolverAddress, manifestHash: fixture.manifest_hash, createTx: readJson("deployments/base-connected-pool.json").createTx, approvalTx: readJson("deployments/base-connected-pool.json").approvalTx, yesStakeTx: readJson("deployments/base-connected-pool.json").yesStakeTx, noStakeTx: readJson("deployments/base-connected-pool.json").noStakeTx, yesTotal: yesTotal.toString(), noTotal: noTotal.toString(), closeTime: fixture.close_time, resolutionAvailableTime: fixture.resolution_available_time, terminalDeadline: fixture.absolute_terminal_deadline, usdcAfter: formatUnits(balanceAfter, 6), status: "FUNDED_BOTH_SIDES" }));
